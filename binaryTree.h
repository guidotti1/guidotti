...
private:
    ...
    int leavesCount(binaryTreeNode<elemType> *p) const;
      ...
      //tree to which p points
    int singleParentCount(binaryTreeNode<elemType> *p);
};
...
template <class elemType>
int binaryTreeType<elemType>::singleParentCount(binaryTreeNode<elemType> *p)
{
   int count = 0;
    ...
    if (p->llink != NULL)
        {
        count += singleParentCount(p->llink);
    }
    if (p->rlink != NULL)
    {
        count += singleParentCount(p->rlink);
    }
...
template<class elemType>
int binaryTreeType<elemType>::singleParentCountRun()
{
